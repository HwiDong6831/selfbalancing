...
ESP32 에는 I2C 컨트롤러(버스)가 **2개** 있고, 컨트롤러마다 7비트 주소 0x00~0x7F(128개)를
다룬다. 그중 0x00~0x07 과 0x78~0x7F 는 예약이라 실제로 쓸 수 있는 건 **0x08~0x77(112개)**.

MPU6050 이 2개였다면 컨트롤러 하나씩 나눠 주소 충돌을 피할 수 있다.
**3개라서 안 된다.** 그래서 mux 로 채널을 갈라 붙인다.

mux 는 여러 입력 중 하나를 골라 출력에 연결하는 스위치다. 대가가 있다:

- 폴링 주기가 짧을수록 **채널 전환 오버헤드**의 비중이 커진다
- 여러 채널과 **동시 통신 불가** — 순차 접근해야 한다

### ESP-IDF I2C API 패턴

`_config_t` = 장치·버스를 만들기 위한 설정 구조체, `_handle_t` = 만들어진 객체(식별자).
ESP-IDF 페리페럴 API 전반이 같은 꼴이다.

```c
i2c_device_config_t cfg = { ... };            // 1. 설정을 채우고
i2c_master_dev_handle_t dev;                  // 2. 핸들(빈 손잡이)을 준비
i2c_master_bus_add_device(bus, &cfg, &dev);   // 3. 설정을 넣으면 핸들이 나온다
i2c_master_transmit(dev, ...);                // 4. 이후엔 핸들로 "이 장치"를 지목
```

> 이 프로젝트는 이후 `i2c_master` 를 버리고 레거시 `driver/i2c.h` 로 이관했다.
> 이유는 [2026-07-21 일지](2026-07-21_i2c-bus-hang.md) 참조.
...
### 2. WHO_AM_I = 0x70 — 클론이 아니라 애초에 MPU6500
- 증상: 채널 스캔에 0x68 뜨는데(장치 존재) `mpuInit()` FAIL
- 원인: 코드가 WHO_AM_I 0x68/0x69만 허용. 이 칩은 **0x70** 반환
- 해결: WHO_AM_I 체크 완화 — 통신 실패(0x00/0xFF)만 거르고 나머지 응답 통과
- 데이터 자체는 정상 (정지 시 az ≈ +1.00g 확인)
- **정정(2026-07-27)**: 처음엔 "MPU6050 클론칩의 특성" 으로 적었으나 틀렸다.
  **0x70 은 MPU6500 의 WHO_AM_I 값이다.** GY-521 계열 모듈이 MPU6050 이름으로 팔리면서
  실제로는 MPU6500/9250 이 올라간 경우가 흔하다. 쓰는 레지스터(가속도·자이로 14바이트
  버스트, `PWR_MGMT_1`)는 두 칩이 같아 동작에는 문제가 없다. 표기만 틀린 것
